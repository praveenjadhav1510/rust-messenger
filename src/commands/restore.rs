...
pub async fn exec(username: &str) -> Result<()> {
    ...
    let response = client
        .restore_account(username, &recovery_code, &keys.public_key_hex)
        .await?;
    ...
    // Fetch the new fingerprint by looking up the restored user
    let lookup_info = client.lookup_user(username).await?;

    // Update profile
    profile.username = Some(username.to_string());
    profile.fingerprint = Some(lookup_info.fingerprint);
